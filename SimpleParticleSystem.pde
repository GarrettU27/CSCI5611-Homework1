...
//     horizontal motion (because you just add the vectors). Make sure your code solving Step 6 does not have that issue, and that the red ball moves diagonally
//     at the same speed it move horizontally or vertically.
...
float COR = 0.7;
// assume 0.5 meter = 5 pixels, so the little spheres have a diameter of
// 1 meter. Then, 9.8 * (5/0.5) = 98
Vec2 gravity = new Vec2(0, 98);

//Initalalize variable
Vec2 pos[] = new Vec2[maxParticles];
Vec2 vel[] = new Vec2[maxParticles];
float[] lifetimes = new float[maxParticles];
...
void update(float dt) {
  ...
  for (int i = 0; i < toGen; i++) {
    ...
    lifetimes[numParticles] = 0;
    numParticles += 1;
  }


  obstacleVel = new Vec2(0, 0);
  if (leftPressed) obstacleVel = obstacleVel.plus(new Vec2(-1, 0));
  if (rightPressed) obstacleVel = obstacleVel.plus(new Vec2(1, 0));
  if (upPressed) obstacleVel = obstacleVel.plus(new Vec2(0, -1));
  if (downPressed) obstacleVel = obstacleVel.plus(new Vec2(0, 1));

  if (obstacleVel.length() > 0) {
    obstacleVel.normalize();
  ...
  for (int i = 0; i <  numParticles; i++) {
    ...
    Vec2 accVel = gravity.times(dt); // Velocity change due to gravity
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)) {
      ...
      Vec2 velNormal = normal.times(dot(vel[i], normal)).minus(normal.times(dot(obstacleVel, normal)).times(COR));
    ...
    if (lifetimes[i] > maxLifetime) {
      ...
      vel[i] = new Vec2(30+random(60), -190-random(10));
      lifetimes[i] = 0;
    }
  }
}
