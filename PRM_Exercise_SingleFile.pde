...
int closestNode(Vec2 point){
  ...
  for (int i = 1; i < numNodes; i++){
    if(nodePos[i].distanceTo(point) < nodePos[closestPoint].distanceTo(point)) {
      closestPoint = i;
    }
  }
  return closestPoint;
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  //the 2 is to add a bit of a safety factor so it looks less like the points are
  //going into the rectangle
  return boxTopLeft.x - 2 < pointPos.x 
    && boxTopLeft.x + boxW + 2 > pointPos.x 
    && boxTopLeft.y - 2 < pointPos.y 
    && boxTopLeft.y + boxH + 2 > pointPos.y;
}
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideRectangle = pointInBox(boxTopLeft, boxW, boxH, randPos);
    while (insideAnyCircle || insideRectangle){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos);
      insideRectangle = pointInBox(boxTopLeft, boxW, boxH, randPos);
    }
    nodePos[i] = randPos;
  }
}
