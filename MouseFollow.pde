...
float currentSpeed = 0; // pixels per second
float acceleration = 20; // pixels per second squared

int currentSecond = 0;

void update(float dt){
  Vec2 mousePos = new Vec2(mouseX, mouseY);
  Vec2 dir = new Vec2(mouseX - pos.x,mouseY - pos.y); //Should be vector pointing from pos to MousePos
  ...
  vel = dir.times(currentSpeed * dt);
  pos.add(vel);
  
  float tolerance = r/10;
  if (abs(pos.x - mouseX) < tolerance && abs(pos.y - mouseY) < tolerance) {
     pos = mousePos; 
     currentSpeed = 0;
  }
}
